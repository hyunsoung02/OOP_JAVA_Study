...
### 💻 제어문(조건문 & 반복문) 특징 증명 실습

보내주신 제어문의 특징을 하나의 비즈니스 시나리오(창고 재고 관리 시스템) 코드로 구현하여 검증했습니다.

* **소스 코드 파일:** [ControlFlowExam.java](./Practice/ControlFlowExam.java)

| 구현한 핵심 기능 | 관련 소스 코드 라인 / 특징 매칭 |
| :--- | :--- |
| **중첩 if문 & switch문** | 관리자 권한(`isManager`) 체크 후 내부에서 세부 등급별 메뉴 활성화로 구조적 중첩 검증 |
| **break vs continue** | 진열대 3번 칸은 `continue`로 건너뛰고, 사과 재고가 소진될 때 `break`로 안쪽 루프 탈출 |
| **이름 붙은 반복문** | `FruitWarehouse:` 라벨을 지정하여, 바나나 재고 고갈 시 전체 바깥 루프를 한 번에 원격 탈출 |
| **do-while문 보장** | 조건식이 처음부터 `false`여도 최초 1회 강제 점검 로그가 출력되는 동작 증명 |
-------------------------------------------------
